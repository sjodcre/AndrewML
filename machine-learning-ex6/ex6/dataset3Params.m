...
%error = size(length(CArr), length(sigmaArr));
for i = 1:length(CArr)
  for j = 1:length(sigmaArr)
    ...
    fprintf(' For C = %f and sigma = %f , error is %f\n',CArr(i),sigmaArr(j),error(i,j));
  endfor
endfor
[minColumnError, minColumnErrorIndex] = min(error);
[minError, minErrorIndex] = min(minColumnError);

C = CArr(minColumnErrorIndex(minErrorIndex));
sigma = sigmaArr(minErrorIndex);
%C = 30;
%sigma = 0.03;
